    ...
    const BarBox = ({ label, value }) => (
        <div className="flex flex-col items-center gap-1.5 flex-1 min-w-[32px]">
            <div className="w-full bg-white/5 rounded-t-md h-24 md:h-28 relative flex items-end justify-center pb-0 overflow-hidden border border-white/5">
                <div className="w-full mx-1 rounded-t-[2px] transition-all duration-500 ease-out flex items-end justify-center pb-1 bg-indigo-500/80 backdrop-blur-sm" style={{ height: `${(value / maxMark) * 100}%` }}>
    ...
    return (
        <div className="w-full max-w-[950px] mx-auto bg-slate-900/60 backdrop-blur-3xl border border-white/10 shadow-2xl shadow-black/40 rounded-[16px] overflow-hidden font-sans text-slate-200 relative">
            <div className="max-h-[85vh] overflow-y-auto no-scrollbar md:max-h-none md:overflow-y-visible">
                ...
                {/* Main content — mobile: always row; desktop: original row layout */}
                <div className="p-4 md:p-5 flex flex-row gap-3 md:gap-5 items-start">

                    {/* Avatar — smaller on mobile to fit beside stats */}
                    ...
                    <div className="flex-grow min-w-0 w-full flex flex-col gap-2">
                        ...
                        <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-1.5 md:gap-3">
                            <h1 className="text-base md:text-2xl lg:text-3xl font-bold text-white leading-tight tracking-tight break-words text-left flex-shrink min-w-0 pr-8 md:pr-0">{student.name}</h1>

                            {/* Roll/Class — inline on mobile, stacked on desktop (original) */}
                            <div className="flex flex-wrap gap-1.5 md:flex-col md:gap-1 flex-shrink-0">
                                <div className="flex items-center gap-1 md:gap-1.5 bg-white/5 border border-white/10 rounded-md px-1.5 md:px-2 py-0.5">
                                    <span className="uppercase text-[7px] md:text-[8px] tracking-wider text-slate-500 font-semibold">Roll</span>
                                    <span className="font-mono text-[11px] md:text-xs text-slate-300">{student.roll_no}</span>
                                </div>
                                <div className="flex items-center gap-1 md:gap-1.5 bg-white/5 border border-white/10 rounded-md px-1.5 md:px-2 py-0.5">
                                    <span className="uppercase text-[7px] md:text-[8px] tracking-wider text-slate-500 font-semibold">Class</span>
                                    <span className="text-[11px] md:text-xs text-slate-300">{formatClassName(student.class)}</span>
                                </div>
                            </div>

                            {/* Stats — same structure, compact on mobile */}
                            <div className="flex gap-2 flex-nowrap flex-shrink-0">
                                <div className="bg-white/5 border border-white/10 rounded-lg px-2 md:px-2.5 py-1 min-w-fit flex flex-col justify-center items-center gap-0 relative overflow-hidden group hover:bg-white/[0.08] transition-all">
                                    ...
                                    <div className="flex items-baseline gap-0.5 md:gap-1 z-10">
                                        <span className="text-base md:text-xl font-bold text-white tracking-tight leading-none">{formatFloat(student.cgpa, 2)}</span>
                                ...
                                <div className="bg-white/5 border border-white/10 rounded-lg px-2 md:px-2.5 py-1 min-w-fit flex flex-col justify-center items-center gap-0 relative overflow-hidden group hover:bg-white/[0.08] transition-all">
                                    ...
                                    <div className="flex items-baseline gap-0.5 md:gap-1 z-10">
                                        <span className="text-base md:text-xl font-bold text-white tracking-tight leading-none">{formatFloat(student.attendance, 1)}</span>
                        ...
                        {/* Ranks — 2x2 grid on mobile, flex row on desktop (original) */}
                            ...
                            {[
                                ...
                                rank.val && (
                                    <div key={idx} className="py-1 px-2 md:px-2.5 bg-white/5 border border-white/10 rounded-md flex items-center justify-between gap-1 md:gap-2 hover:bg-white/10 transition-colors md:flex-1 md:min-w-[120px]">
                                        ...
                                        <span className="font-mono text-[11px] md:text-xs font-bold text-white">#{rank.val}</span>
                                    </div>
                                )
                            ))}
                        </div>
                        ...
                        {/* Top Subjects — MOBILE: compact square grid */}
                        {topSubjects.length > 0 && (
                            <div className="grid grid-cols-2 gap-1.5 w-full sm:hidden">
                                {topSubjects.map((sub, idx) => (
                                    <div key={idx} className="aspect-square flex flex-col items-center justify-center p-1.5 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
                                        <span className="text-[7px] uppercase tracking-wider text-slate-500 font-bold leading-none mb-0.5">Best #{idx + 1}</span>
                                        <span className="text-[10px] font-medium text-slate-200 text-center leading-tight line-clamp-2" title={sub.subject_name}>{shortName(sub.subject_name)}</span>
                                        <div className="flex items-baseline gap-1 mt-0.5">
                                            <span className="text-sm font-bold text-white leading-none">{sub.total_marks}</span>
                                            <span className={`text-[8px] font-bold ${sub.rank <= 3 ? 'text-amber-400' : 'text-slate-500'}`}>#{sub.rank || '-'}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {subjects.length > 0 && (
                    <div className="border-t border-white/5 mt-2 bg-black/20 p-3 md:p-6">
                        ...
                        {/* Shared bar graph */}
                        {activeSubject && (
                            <div className="bg-white/[0.02] rounded-lg border border-white/5 p-3 md:p-4">
