...
export default function StudentIDCard({ student, loading, error, onClose }) {
    ...
    return (
        <div className="w-full max-w-[1000px] mx-auto bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] font-mono text-black relative z-[60] max-h-[85vh] md:max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {subjects.length > 0 && (
                    <div className="p-4 md:p-6 bg-[#f0f0f0] border-t-4 border-black">
                        ...
                        {/* Scrolling Tabs - 2 Rows on Mobile */}
                        <div className="grid grid-rows-2 grid-flow-col gap-2 overflow-x-auto pb-4 mb-2 custom-scrollbar md:flex md:flex-row md:overflow-x-auto">
            ...
            {showAnalysis && analysisData && (
                <div className="absolute inset-0 z-[70] bg-[#ffde00] flex flex-col animate-in slide-in-from-bottom-full duration-300">
                    <div className="border-b-4 border-black p-4 flex justify-between items-center bg-white sticky top-0 z-10 shadow-md flex-shrink-0">
                    ...
                    <div className="p-4 md:p-8 overflow-y-auto flex-1 pb-20">
                        ...
                        <div className="space-y-2">
                            {/* Grid Header */}
                            <div className="grid grid-cols-[1fr_3rem_3rem_3rem_4rem] md:grid-cols-[1fr_4rem_4rem_4rem_5rem] gap-2 items-center border-b-4 border-black pb-2 mb-4 bg-white p-2">
                                <span className="font-black text-sm uppercase">Subject</span>
                                <span className="font-black text-xs uppercase text-right">Marks</span>
                                <span className="font-black text-xs uppercase text-right">GP</span>
                                <span className="font-black text-xs uppercase text-right">C</span>
                                <span className="font-black text-xs uppercase text-center">CxGP</span>
                            </div>

                            {/* Grid Rows */}
                            {analysisData.breakdown.map((row, idx) => (
                                <div key={idx} className="grid grid-cols-[1fr_3rem_3rem_3rem_4rem] md:grid-cols-[1fr_4rem_4rem_4rem_5rem] gap-2 items-center border-b-2 border-black pb-2 border-dashed px-2 bg-white/50 hover:bg-white transition-colors">
                                    <span className="font-bold text-xs md:text-sm uppercase truncate" title={row.subject}>{row.subject}</span>
                                    <div className="font-mono font-bold text-right">{formatFloat(row.marks, 2)}</div>
                                    <div className={`font-mono font-bold text-right ${row.gradePoint === 0 ? 'text-red-600 bg-red-100' : ''}`}>{formatFloat(row.gradePoint, 2)}</div>
                                    <div className="font-mono font-bold text-right">{row.credits}</div>
                                    <div className="font-mono font-black bg-[#ffde00] border-2 border-black text-center py-1">{row.weightedPoint}</div>
                                </div>
                            ))}
                        </div>
                    </div>
