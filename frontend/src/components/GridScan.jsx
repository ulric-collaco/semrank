...
import {
    Vector2, Vector3, MathUtils, WebGLRenderer,
    SRGBColorSpace, NoToneMapping, ShaderMaterial,
    Scene, OrthographicCamera, Mesh, PlaneGeometry, Color
} from 'three';
import './GridScan.css';
...
export const GridScan = ({
    ...
    useEffect(() => {
        ...
        const uniforms = {
            iResolution: { value: new Vector3(container.clientWidth, container.clientHeight, renderer.getPixelRatio()) },
            iTime: { value: 0 },
        ...
        if (enablePost) {
            ...
            const chroma = new ChromaticAberrationEffect({ offset: new Vector2(chromaticAberration, chromaticAberration), radialModulation: true, modulationOffset: 0.0 });
    ...
    useEffect(() => {
        ...
        const handler = e => {
            ...
            const beta = e.beta ?? 0;
            lookTarget.current.set(MathUtils.clamp(gamma / 45, -1, 1), MathUtils.clamp(-beta / 30, -1, 1));
    ...
    // Face-api model loading — ONLY when webcam is enabled (dynamic import)
    useEffect(() => {
    ...
    // Face detection loop — ONLY when webcam is enabled (dynamic import)
    useEffect(() => {
                ...
                if (ts - lastDetect >= 33) {
                    ...
                    try {
                        ...
                        if (res && res.detection) {
                            ...
                            const look = new Vector2(Math.tanh(median(bufX.current)), Math.tanh(median(bufY.current)));
                            ...
                            const dL = dist2(tip, jaw[3] || jaw[2]);
                            const dR = dist2(tip, jaw[13] || jaw[14]);
                            const eyeDist = Math.hypot(rc.x - lc.x, rc.y - lc.y) + 1e-6;
                            medianPush(bufYaw.current, Math.tanh(MathUtils.clamp((dR - dL) / (eyeDist * 1.6), -1, 1)), 5);
